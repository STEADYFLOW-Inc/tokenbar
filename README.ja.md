...
- **初回簡単セットアップ** — 初回起動時にウィザードが起動。Claude ディレクトリ（認証情報の存在をリアルタイム検証）・モニター・位置・自動起動をまとめて設定。
- **設定ウィンドウ**（左クリック）— ウィジェットのライブプレビュー、Windows のディスプレイ設定風のビジュアルモニターピッカー、タイトル・数値テキスト・リセット時刻の表示切替、バー選択（5時間セッション / 週間全体 / 週間モデル別）。
...
- **自動フォールバック** — メーターは常に API 値を表示。API が一時的に失敗した場合は直前の正常な API 値を「◯:◯◯時点」ラベル付きで表示し続ける（再起動をまたいでも保持）。`~/.claude/projects/**/*.jsonl` のローカル推計（ccusage 方式）はツールチップと `--dump` の参考用途のみに。
...
- **テーマ同期** — Windows のタスクバー配色（ライト / ダーク / アクセントカラー表示）を自動検出してカードの色を合わせる。テーマ切替にもライブで追従。
...
| キー | 既定値 | 説明 |
|------|--------|------|
| `tokenLimit` | `200000` | `--dump` とツールチップの参考推計のみで使用。メーター表示は常に API 値 |
| `includeCacheRead` | `false` | `--dump` とツールチップの参考推計のみで使用。メーター表示は常に API 値 |
| `refreshSec` | `120` | データ更新間隔（秒）。短くしすぎると共有の usage エンドポイントが一時的に 429 を返すことがある |
...
| 表示されるパーセンテージがおかしい | インジケーターのドットを確認してください。アンバー色は API が一時的に利用できず、直前の正常な API 値を「◯:◯◯時点」ラベル付きで表示している状態です。API が復帰すると自動的に更新されます。 |
