...
- **クリーンモード** — 非公式 API への通信を完全にオフにできます（ネットワーク通信なし・認証情報の読み込みなし）。メーターはローカルのトランスクリプト推計を使用し、上限値は過去7日間のデータから自動キャリブレーションされます。設定ウィンドウのトグル、または `config.json` で `useApi: false` を設定することで有効になります。
...
- **テーマ同期＆完全透過** — Windows のタスクバー配色（ライト / ダーク / アクセントカラー表示）を自動検出してカードの色を合わせ、テーマ切替にライブ追従。さらに角丸カードの外側はピクセル単位で完全透明なので、本物の半透明タスクバーがそのまま透けて見える（透明部分のクリックはタスクバーに透過）。
...
| キー | 既定値 | 説明 |
|------|--------|------|
| `useApi` | `true` | `true` = OAuth usage API を使用（正確）、`false` = クリーンモード: ネットワーク通信なし・認証情報の読み込みなし、ローカル推計を使用 |
| `tokenLimit` | `200000` | ローカル推計で使用するトークン上限。`0` = 過去7日間のデータから自動キャリブレーション（クリーンモード / フォールバック推計で使用） |
...
`useApi` を `false` に設定（または設定ウィンドウでオフに切り替え）すると、TokenBar は API への通信を一切行いません。ネットワークリクエストと `~/.claude/.credentials.json` の読み込みが両方スキップされます。メーターはローカルの JSONL 推計のみで動作し、トークン上限は過去7日間のトランスクリプトデータから自動キャリブレーションされます（`tokenLimit: 0` で手動設定不要）。
...
- **設置場所**: `TokenBar.exe` は自分のユーザーだけが書き込めるフォルダ（例: `%LOCALAPPDATA%\TokenBar\`）に置いてください。Windows の実行ファイル全般と同様、誰でも書き込めるフォルダで実行すると DLL 検索順ハイジャックのリスクがあります。
- **クリーンモード**（`useApi: false`）を有効にすると、すべてのネットワーク通信と認証情報の読み込みが完全に無効化されます。メーターはローカルの JSONL 推計のみで動作します。
