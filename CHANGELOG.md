...
### Security
- Open-config now launches notepad via its absolute System32 path (no PATH lookup) with a quote-sanitized argument
- GitHub Actions pinned to commit SHAs; build workflow restricted to read-only token
- Strict TLS floor (1.2/1.3 only) instead of additive protocol flags
- README documents the recommended install location (user-writable-only folder) against DLL search-order hijacking

## [1.7.0] - 2026-07-16

### Added
- Clean mode (`useApi: false` / settings toggle): no network access and no credential reads — the meter runs purely on the local JSONL estimate
- Auto-calibrated token limit (`tokenLimit: 0`): uses the largest 5-hour block of the past 7 days as the assumed limit
