...
- **Theme sync & true transparency** — matches the Windows taskbar automatically (light / dark / accent-colored, live re-detection), and everything outside the rounded card is per-pixel transparent, so the real translucent taskbar shows through (transparent areas are click-through).
